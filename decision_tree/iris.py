...
class Iris:
    ...
    def execute(self):
        ...
        iris = load_iris()
        df = pd.DataFrame(iris.data, columns=iris.feature_names)    # pandas => 시트 만듦
        # print(df.head())
        # print(df.columns)
        """
       ...
       'petal width (cm)', 'species', 'is_train']
        ...
        # print(df.columns)
        df['is_train'] = np.random.uniform(0, 1, len(df)) <= .75 # train set 75%    = >  지도학습
        # print(df.columns)
        train, test = df[df['is_train']==True], df[df['is_train']==False]
        features = df.columns[:4] # 앞에서부터 4번제 컬럼(=피쳐)까지 추출
        # print(features)
        ...
        #print(clf.predict_proba(test[features])[0:10]) # 확률 예측
        # 정확도 평가
